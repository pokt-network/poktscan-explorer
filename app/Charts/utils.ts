...
export function hashStringToColor(id: string): string {
  ...
  const hue = Math.abs(hash * 40) % 360; // 137 ensures better color spread
  const saturation = 60 + (Math.abs(hash * 29) % 70); // Saturation between 60-90%
  const lightness = 40 + (Math.abs(hash * 27) % 20); // Lightness between 40-60%
